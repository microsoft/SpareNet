...
template <typename scalar_t> struct p2i_max_forward_kernel {
  XDEVICE void operator()(int id,
                          ...
                          int32_t *out_point_ids, // batch x channels x out_h x
...
template <typename scalar_t> struct p2i_max_backward_kernel {
  ...
  operator()(int id,
             ...
             scalar_t *background_grad,      // batch x channels x out_h x out_w
             int batch, int npoints, int channels, int kernel_kind,
             scalar_t kernel_radius, int out_h, int out_w) const {
    ...
    int channel_id = id % channels;
    id /= channels;
    int batch_id = id % batch;

    scalar_t out_grad_value = out_grad[index];

    int point_id = out_point_ids[index];
    ...
    scalar_t dx = x - point_x, dy = y - point_y;
    ...
    int point_feature_offset = point_id * channels + channel_id;
    ...
    case 0: // cos
      // weight = cos(r * M_PI / kernel_radius) * 0.5 + 0.5;
      scalar_t k = weight_grad_value * sin(r * M_PI / kernel_radius) * 0.5 *
                   M_PI / kernel_radius / max(r, static_cast<scalar_t>(1e-10));
      point_y_grad = k * dy;
      point_x_grad = k * dx;
      break;
    }
